...
alinesvp0 <-function(pos,up,down,width_)
{
grid.lines(c(.5,.5), c(.5 + down,.5 + up))
grid.lines(c(.5-width_,.5+width_), c(.5+ up,.5 + up))
grid.lines(c(.5-width_,.5+width_), c(.5 + down,.5 + down))
}
